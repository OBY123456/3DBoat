...
namespace Crest
{
    ...
    public class OceanRenderer : MonoBehaviour
    {
        ...
        public Slider OceanLightSlider;
...
#pragma warning restore 414
        ...
        bool VerifyRequirements()
        {
            ...
            {
                Debug.LogError("Crest requires a Lightweight Render Pipeline asset to be configured in the graphics settings - please refer to Unity documentation or setup instructions.", this);
                return false;
            }

            if (_verifyOpaqueAndDepthTexturesEnabled)
            {
                if (!(GraphicsSettings.renderPipelineAsset as LightweightRenderPipelineAsset).supportsCameraOpaqueTexture)
                {
                    Debug.LogWarning("To enable transparent water, the 'Opaque Texture' option must be ticked on the Lightweight Render Pipeline asset.", this);
                }

                if (!(GraphicsSettings.renderPipelineAsset as LightweightRenderPipelineAsset).supportsCameraDepthTexture)
                {
                    Debug.LogWarning("To enable transparent water, the 'Depth Texture' option must be ticked on the Lightweight Render Pipeline asset.", this);
                }
            }
...
#if UNITY_EDITOR
        void UpdateVerifyLWRPVersion()
        {
            if (_request != null && _request.IsCompleted)
            {
                foreach (var pi in _request.Result)
                {
                    if (pi.name == "com.unity.render-pipelines.lightweight")
                    {
                        var requiredVersion = new System.Version(5, 16, 1);
                        if (System.Version.Parse(pi.version) < requiredVersion)
                        {
                            Debug.LogWarning("Crest requires LWRP " + requiredVersion + " or later, please update. The ocean depth cache fails to render prior to 5.13. I've had issues where the environment reflections did not work in standalone builds prior to 5.16.1.", this);
                        }
        ...
        public void UpdateOceanLightData()
        {
            OceanMaterial.SetFloat("_Specular", OceanLightSlider.value);
            OceanMaterial.SetFloat("_CausticsTextureAverage", 0.37f-(0.37f-0.07f)* OceanLightSlider.value);
            //if (OceanLightSlider.value < 0.1f)
            //{
            //    OceanMaterial.SetFloat("_CausticsTextureAverage", 0.37f);
            //}
            //else
            //{
            //    OceanMaterial.SetFloat("_CausticsTextureAverage", 0.07f);
            //}
        }
